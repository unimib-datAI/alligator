...
class FeauturesExtraction:
    ...
    async def _compute_similarity_between_ne_cells(self, subj_cell, obj_cell, nNE_cells):
        async with self._lamAPI.semaphore:
            ...
            subj_id_candidates = [
                candidate["id"] for candidate in subj_cell.candidates() if candidate["id"] not in self._cache_obj
            ]
            ...
            for subj_candidate in subj_cell.candidates():
                ...
                for obj_candidate in obj_cell.candidates():
                    ...
                    string_similarity_features = [
                        obj_candidate["features"][f] for f in ["ed_score", "jaccard_score", "jaccardNgram_score"]
                    ]
                    ...
                    string_similarity_features = [
                        subj_candidate["features"][f] for f in ["ed_score", "jaccard_score", "jaccardNgram_score"]
                    ]
    ...
    async def _match_lit_cells(self, subj_cell, obj_cell, row, nLIT_cells):
        async with self._lamAPI.semaphore:
            ...
            subj_id_candidates = [
                candidate["id"] for candidate in subj_cell.candidates() if candidate["id"] not in self._cache_lit
            ]
            if len(subj_id_candidates) > 0:
            ...
            for subj_candidate in subj_cell.candidates():
                ...
                p_subj_lit_all_datatype = metrics.compute_similarity_between_string_token_based(
