...
class FeauturesExtraction:
    ...
    async def compute_feautures(self):
        ...
        for row in self._rows:
            ...
            cells = row.get_cells()
            tasks.append(
                asyncio.create_task(self._compute_rows(ne_cells, lit_cells, cells, row))
            )
        ...
        for ne_cell in ne_cells:
            for cell in cells:
                if cell == ne_cell:
                    ...
                    await self._compute_similarity_between_ne_cells(
                        ne_cell, cell, len(ne_cells)
                    )
                    # tasks.append(asyncio.create_task(self._compute_similarity_between_ne_cells(ne_cell, cell, len(ne_cells))))
        # await asyncio.gather(*tasks)
    ...
    async def _compute_similarity_between_ne_cells(
        self, subj_cell, obj_cell, nNE_cells
    ):
        async with self._lamAPI.semaphore:

            subj_id_candidates = [
                candidate["id"]
                for candidate in subj_cell.candidates()
                if candidate["id"] not in self._cache_obj
            ]
            ...
            for subj_candidate in subj_cell.candidates():
                ...
                async with self._lock_obj:
                    if id_subject not in self._cache_obj:
                        subj_candidates_objects = subjects_objects.get(
                            id_subject, {}
                        ).get("objects", {})
                ...
                obj_score_max = 0
                objects_itersection = objects_set.intersection(
                    set([candidate["id"] for candidate in obj_cell.candidates()])
                )
                for obj_candidate in obj_cell.candidates():
                    ...
                    string_similarity_features = [
                        obj_candidate["features"][f]
                        for f in ["ed_score", "jaccard_score", "jaccardNgram_score"]
                    ]
                    p_subj_ne = round(
                        sum(string_similarity_features)
                        / len(string_similarity_features),
                        3,
                    )
                    ...
                    for predicate in subj_candidates_objects[id_object]:
                        subj_candidate["matches"][str(obj_cell._id_col)].append(
                            {"p": predicate, "o": id_object, "s": round(p_subj_ne, 3)}
                        )
                        subj_candidate["predicates"][str(obj_cell._id_col)][
                            predicate
                        ] = p_subj_ne
                subj_candidate["features"]["p_subj_ne"] += round(
                    obj_score_max / nNE_cells, 3
                )
            ...
            for obj_candidate in obj_cell.candidates():
                ...
                obj_candidate["features"]["p_obj_ne"] += round(
                    object_rel_score_buffer[id_object] / nNE_cells, 3
                )
    ...
    async def _match_lit_cells(self, subj_cell, obj_cell, row, nLIT_cells):
        async with self._lamAPI.semaphore:
            ...
            def get_score_based_on_datatype(valueInCell, valueFromKG, datatype):
                ...
                if datatype == "NUMBER":
                    score = metrics.compute_similarty_between_numbers(
                        valueInCell, valueFromKG.lower()
                    )
                elif datatype == "DATETIME":
                    score = metrics.compute_similarity_between_dates(
                        valueInCell, valueFromKG.lower()
                    )
                elif datatype == "STRING":
                    score = metrics.compute_similarity_between_string(
                        valueInCell, valueFromKg.lower()
                    )
                return score
            ...
            subj_id_candidates = [
                candidate["id"]
                for candidate in subj_cell.candidates()
                if candidate["id"] not in self._cache_lit
            ]
            if len(subj_id_candidates) > 0:
            ...
            for subj_candidate in subj_cell.candidates():
                ...
                async with self._lock_lit:
                    if id_subject not in self._cache_lit:
                        subj_literals = subjects_literals.get(id_subject, {}).get(
                            "literals", {}
                        )
                ...
                p_subj_lit_all_datatype = (
                    metrics.compute_similarity_between_string_token_based(
                        lit_string, row_text_lit
                    )
                )
                p_subj_lit_row = metrics.compute_similarity_between_string_token_based(
                    lit_string, row_text_all
                )
                subj_candidate["features"]["p_subj_lit_all_datatype"] = round(
                    p_subj_lit_all_datatype, 3
                )
                ...
                if (
                    new_datatype not in subj_literals
                    or len(subj_literals[new_datatype]) == 0
                ):
                    continue
                ...
                for predicate in subj_literals[new_datatype]:
                    for valueFromKg in subj_literals[new_datatype][predicate]:
                        p_subj_lit = get_score_based_on_datatype(
                            obj_cell.content, valueFromKg, datatype
                        )
                        p_subj_lit = round(p_subj_lit, 3)
                        if p_subj_lit > 0:
                            subj_candidate["matches"][str(obj_cell._id_col)].append(
                                {"p": predicate, "o": valueFromKg, "s": p_subj_lit}
                            )
                            if p_subj_lit > max_score:
                                max_score = p_subj_lit
                            if (
                                predicate
                                not in subj_candidate["predicates"][
                                    str(obj_cell._id_col)
                                ]
                            ):
                                subj_candidate["predicates"][str(obj_cell._id_col)][
                                    predicate
